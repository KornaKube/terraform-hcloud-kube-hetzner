...
* **`cluster_ipv6_cidr` / `service_ipv6_cidr` (String, Optional):**
  ...
  * **Considerations:** Static control-plane and agent nodes advertise `node-ip` values matching the configured cluster CIDR families: private IPv4 for IPv4 clusters, public IPv6 for IPv6-only clusters, and private IPv4 plus public IPv6 for dual-stack clusters. Existing IPv4-only nodes may need controlled re-registration/replacement after enabling dual-stack so Kubernetes assigns IPv6 PodCIDRs. `nat_router` is private IPv4-only and is rejected with IPv6 cluster CIDRs in this release. Cilium native routing is rejected with IPv6 cluster CIDRs; use the default tunnel mode. Standard private-network IPv6/dual-stack does not open direct node-to-node public IPv6 firewall paths. Active autoscaler nodepools are rejected for standard private-network IPv6/dual-stack clusters until autoscaler cloud-init can derive IPv6/dual-stack `node-ip` values at boot; use static agents or the experimental public-overlay path with a transport family that includes every enabled cluster CIDR family.

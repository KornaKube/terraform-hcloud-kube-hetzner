...
* **`cluster_ipv6_cidr` / `service_ipv6_cidr` (String, Optional):**
  ...
  * **Considerations:** Static control-plane and agent nodes advertise dual-stack `node-ip` values from their private IPv4 and public IPv6 addresses. Existing IPv4-only nodes may need controlled re-registration/replacement after enabling dual-stack so Kubernetes assigns IPv6 PodCIDRs. Active autoscaler nodepools are rejected for standard private-network dual-stack clusters until autoscaler cloud-init can derive dual-stack `node-ip` values at boot; use static agents or the experimental public-overlay path.

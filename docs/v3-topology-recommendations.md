...
| Cilium dual-stack | `cni_plugin = "cilium"` plus dual-stack cluster/service CIDRs | Best advanced CNI path for IPv4/IPv6; plan-validated for static nodes, live E2E pending | Flannel/Calico when you need Cilium-only features; active autoscaler nodepools, `nat_router`, Robot nodes, and Tailscale transport with IPv6 CIDRs are rejected at plan time |
